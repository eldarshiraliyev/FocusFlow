    ...
    chrome.storage.sync.get('spentTimes', (data) => {
            ...
            div.innerHTML = `
                <div class="site-info">
                    ...
                    <span class="site-meta">
                        ⏱️ ${currentSpent}/${site.time} min ${isBlockedText(isLimitHit)}
                    </span>
                </div>
        ...
        document.querySelectorAll('.remove-site').forEach(btn => {
            btn.addEventListener('click', (e) => {
                blockedSites.splice(e.target.getAttribute('data-index'), 1);
                saveData();
            });
        });
    });
...
function isBlockedText(hit) {
    return hit ? "<b style='color:#ff4757; font-weight:700;'>(BLOCKED)</b>" : "";
}
...
function calculatePremiumStats(isActive) {
    if (!isActive) {
        ...
        statToday.innerHTML = `0 <span>min</span>`;
        statTodaySub.textContent = "Top: None";
    ...
    chrome.storage.sync.get('spentTimes', (data) => {
        ...
        statToday.innerHTML = `${totalSpent} <span>min</span>`;
        if (topSite !== "None") {
...
// Artıq bu hissə tamamilə təhlükəsizdir və xəta verməyəctedir
if (resetBtn) {
